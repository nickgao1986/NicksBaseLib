...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
		...
		if (null != mOnLastItemVisibleListener) {
			if (totalItemCount >= mLastItemCount) {
				mLastItemVisible = (firstVisibleItem + visibleItemCount >= totalItemCount - mLastItemCount);
			} else {
				mLastItemVisible = (totalItemCount > 0) && (firstVisibleItem + visibleItemCount >= totalItemCount - 1);
			}
		}
		if (null != mOnFirstItemVisibleListener) {
			if (totalItemCount >= mFirstItemCount) {
				mFirstItemVisible = (firstVisibleItem <= mFirstItemCount);
			} else {
	...
	public final void onScrollStateChanged(final AbsListView view, final int state) {
		 ...
		 * visible.
		 */
		if ((state == OnScrollListener.SCROLL_STATE_IDLE || ((Build.VERSION.SDK_INT < 11) && (SCROLL_STATE_FLING == state)))) {
			if (null != mOnLastItemVisibleListener && mLastItemVisible) {
				mOnLastItemVisibleListener.onLastItemVisible();
			} else if (null != mOnFirstItemVisibleListener && mFirstItemVisible) {
	...
	public final void setOnLastItemVisibleListener(OnLastItemVisibleListener listener) {
		setOnLastItemVisibleListener(listener, 1);
	}

	public final void setOnLastItemVisibleListener(OnLastItemVisibleListener listener, int lastItemCount) {
		mOnLastItemVisibleListener = listener;
		this.mLastItemCount = lastItemCount > 1 ? lastItemCount : 1;
	}

	public final void setOnFirstItemVisibleListener(OnFirstItemVisibleListener listener, int firstItemCount) {
		mOnFirstItemVisibleListener = listener;
		this.mFirstItemCount = firstItemCount > 1 ? firstItemCount : 1;
	}
...
//					Log.d("PullToRefreshAdapter", String.format("lastVisibleChild getBottom=%d  adapterview getpaddingbottom=%d  adapterview getbottom=%d",lastVisibleChild.getBottom(),mRefreshableView.getPaddingBottom(),mRefreshableView.getBottom()));
