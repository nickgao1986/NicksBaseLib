...
import com.handmark.pulltorefresh.library.internal.PullToRefreshOptions;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    // Fields
    // ===========================================================
    protected final PullToRefreshOptions mPullToRefreshOptions = new PullToRefreshOptions();
    ...
    protected float mFriction = FRICTION;
    ...
    public final boolean demo() {
        if (mMode.showHeaderLoadingLayout() && isReadyForPullStart()) {
            if (!isSlide) {
                ...
                smoothScrollToAndBack(-getRefreshHeight());
            }
            return true;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    if (isSlide && mState == State.RELEASE_TO_SLIDE && (null != mOnSlideListener)) {
                        setState(State.SLIDE_DOING);
                        return true;
                    }
    ...
    protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
        LoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode,
                getPullToRefreshScrollDirection(), attrs, isSlide);
    ...
    public void setHeaderBackground(@DrawableRes int color) {
        mHeaderLayout.setBackgroundResource(color);
    }
                ...
                switch (mCurrentMode) {
                    ...
                    case PULL_FROM_START:
                        if (!isSlide) {
                            ...
                            smoothScrollTo(-getRefreshHeight(), listener);
                        }
                        break;
                    case PULL_FROM_END:
    ...
    protected void refreshLoadingViewsSize() {
        Log.i(LOG_TAG, "refreshLoadingViewsSize");
        ...
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
                if (mMode.showHeaderLoadingLayout()) {
                    ...
                    pLeft = pLeft - maximumPullScroll;
                }

                if (mMode.showFooterLoadingLayout()) {
                    mFooterLayout.setWidth(maximumPullScroll);
                    pRight = pRight - maximumPullScroll;
                }
                break;
            ...
            case VERTICAL:
                if (mMode.showHeaderLoadingLayout()) {
                    ...
                    pTop = pTop - maximumPullScroll;
                }

                if (mMode.showFooterLoadingLayout()) {
                    mFooterLayout.setHeight(maximumPullScroll);
                    pBottom = pBottom - maximumPullScroll;
                }
                break;
        }
    ...
    protected final void refreshRefreshableViewSize(int width, int height) {
        ...
        // this layout
        if (DEBUG) {
            Log.d(LOG_TAG, "refreshRefreshableViewSize(width,height)");
        }
    ...
    protected  void setHeaderScroll(int value) {
        ...
        if (mLayoutVisibilityChangesEnabled) {
            if (value < 0) {
                ...
                if (mPullToRefreshOptions.mPullDownLimit != 0 && value < mPullToRefreshOptions.mPullDownLimit) {
                    value = mPullToRefreshOptions.mPullDownLimit;
                }
            } else if (value > 0) {
                mFooterLayout.setVisibility(View.VISIBLE);
                if (mPullToRefreshOptions.mPullDownLimit != 0 && value > mPullToRefreshOptions.mPullUpLimit) {
                    value = mPullToRefreshOptions.mPullDownLimit;
                }
            } else {
    ...
    public final void setPullLimit(int pullLimit, PullToRefreshBase.Mode mode) {
        if (mode.showHeaderLoadingLayout())
            mPullToRefreshOptions.mPullDownLimit = pullLimit;
        if (mode.showFooterLoadingLayout())
            mPullToRefreshOptions.mPullUpLimit = pullLimit;
    }

    public final void setPullPadding(int paddingLeft, int paddingTop, int paddingRight, int paddingBottom) {
    ...
    public final int getPullDownLimit() {
        if (mPullToRefreshOptions == null)
            return 0;
        return mPullToRefreshOptions.mPullDownLimit;
    }

    public final int getPullUpLimit() {
        if (mPullToRefreshOptions == null)
            return 0;
        return mPullToRefreshOptions.mPullUpLimit;
    }
    ...
    public void setPullBackground(Drawable drawable, PullToRefreshBase.Mode mode) {
        if ((mHeaderLayout != null) && (mode.showHeaderLoadingLayout()))
            mHeaderLayout.setBackgroundDrawable(drawable);
        if ((mFooterLayout != null) && (mode.showFooterLoadingLayout()))
    ...
    public void setPullAnimationEnabled(boolean animation, PullToRefreshBase.Mode mode) {
        if ((mHeaderLayout != null) && (mode.showHeaderLoadingLayout()))
            mHeaderLayout.setPullAnimationEnabled(animation);
        if ((mFooterLayout != null) && (mode.showFooterLoadingLayout()))
            mFooterLayout.setPullAnimationEnabled(animation);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrFrictionValue)) {
            mFriction = a.getFloat(R.styleable.PullToRefresh_ptrFrictionValue, FRICTION);
        }

        if (a.hasValue(R.styleable.PullToRefresh_ptrSlide)) {
            isSlide = a.getBoolean(R.styleable.PullToRefresh_ptrSlide, false);
        }
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            float scale = Math.abs(newScrollValue) / (float) refreshDimension;
            float slideScale = Math.abs(newScrollValue) / (float) itemDimension;
            switch (mCurrentMode) {
                case PULL_FROM_END:
            ...
            if (!isSlide) {
                ...
                if (mState != State.PULL_TO_REFRESH && refreshDimension >= Math.abs(newScrollValue)) {
                    setState(State.PULL_TO_REFRESH);
                } else if (mState == State.PULL_TO_REFRESH && refreshDimension < Math.abs(newScrollValue)) {
                    setState(State.RELEASE_TO_REFRESH);
                } else if (mState == State.RELEASE_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
                    setState(State.RELEASE_TO_SLIDE);
                } else if (mState == State.RELEASE_TO_SLIDE && itemDimension > Math.abs(newScrollValue) && refreshDimension < Math.abs(newScrollValue)) {
                    setState(State.RELEASE_TO_REFRESH);
                } else if (mState == State.RELEASE_TO_SLIDE && refreshDimension >= Math.abs(newScrollValue)) {
                    setState(State.PULL_TO_REFRESH);
                }
            }
        }
    ...
    protected int getMaximumPullScroll() {
        if (DEBUG) {
            Log.d(LOG_TAG, "getMaximumPullScroll");
        }
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
...
//					return MEITUN;
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs, boolean isSlide) {
            switch (this) {
                case ROTATE:
                default:
                    return new RotateLoadingLayout(context, mode, scrollDirection, attrs, isSlide);
                case FLIP:
                    return new FlipLoadingLayout(context, mode, scrollDirection, attrs, isSlide);
//                case MEITUN_FULL:
//                    return new MtRotateLoadingLayout(context, mode, scrollDirection, attrs, isSlide, true);
            }
        }
    }
    ...
    /**
     * Simple Listener that allows you to be notified when the user has scrolled
     * to the first of the AdapterView. See (
    ...
    public interface OnFirstItemVisibleListener {
        ...
        /**
         * Called when the user has scrolled to the first of the list
         */
    ...
    public final int getRefreshHeight() {
        if (isSlide) {
            return mHeaderLayout.getRefreshHeight();
        } else {
            return getHeaderSize();
        }
    }

    protected final void onPullSlide(float scaleOfLayout) {
        if (isSlide) {
            mHeaderLayout.onPullSlide(scaleOfLayout);
        }
    }

    protected final void onReleaseToSlide() {
        if (isSlide) {
            mHeaderLayout.releaseToSlide();
        }
    }

    protected final void onSliding() {
        if (isSlide) {
            callSlideListener();
        }
    }

    protected final void callSlideListener() {
        if (isSlide && null != mOnSlideListener) {
    ...
    public final boolean isSliding() {
        return isSlide && mState == State.SLIDE_DOING;
    }

    protected OnSlideListener<T> mOnSlideListener;

    public interface OnSlideListener<V extends View> {
        void onSlide(final PullToRefreshBase<V> refreshView);
    }

    public final void setOnSlideListener(OnSlideListener<T> listener) {
